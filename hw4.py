...
dis=pygame.display.set_mode((dis_width,dis_hieght))
#pygame.display.set_caption('Snake game by Jamir Jackson')
...
def game_loop():
	...
	white not game_over:
		while game_close == True:
			dis.fill(white)
			message("you lose, Press q to quit or C to continue)
			...
			y1 += y1_change
			dis.fill(blue)
			pygame.draw.rect(dis,green,[foodc,foody,snake_block]
			snake_head = []
			...
			our_snake(snake_block, snak_list)
			the_score(length_of_snake-1)
			...
			if x1 == foodx and y1 == foody:
				foodx = roudn(random.randrange(0, dis_width-snake_block)
				foody = round(random.randrange(0, dis_height-snake_block)
				length_of_snake += 1
			
			print("the score:" = str(length _of_snake  - 1))
			
		clock.tick(snake_speed)
		
	parser = argparse.ArgumentParser()
	parser.add_argument ('--id', type=str, default='annonymous')
	parser.add_argument ('--file', type=str, default='file.txt')
	args = parser.parse_args()
	scorefile = open(args.file, 'a')
	scorefile.write(args.file, 'a')
